...
var Task = _proto.Task = function(taskName) {
    ...
    this.on('done', function(err, step) {
        if(err) {
            this._errHandler.call(step, err);
        } else {
            if(step._index !== this._steps.length) step.next(step._result);
            else this.emit('done');
        }
    });

    this.on('timeout', this._timeoutHandler.bind(this));
};
...
Task.prototype._run = function(index) {
    ...
    // else throw new Error('Step handler was not defined.');
