...
_proto.done = function(err, callback) {
...
// maybe `promises` better?
_proto.result = _proto.fulfill = function(result) {
...
_proto.jump = function(step) {
    ...
    var targetStep = function() {
        ...
        if('number' === type) return task._getStep(step < 0 ? currIndex + step : step - 1);
...
_proto.parallel = function(arr, iterator, callback) {
    var root = this;
    var completed = 0;
    var callback = util.isFunction(callback) ? callback : this.done;
    var result = [];
    var isUndefined = util._isUndefined;

    util._.each(arr, function(x, i) {
        iterator(x, util.once(function(err, r) {
            if(err) {
                callback(err);
            } else {
                !isUndefined(r) && result.push(r);
                if(++completed >= arr.length) {
                    callback.apply(root, [null, result]);
                }
            }
        }));
    });
