        ...
        var handle = function(err) {
            if(err) {
                ...
                if(isUndefined(next)) {
                    ...
                    if(!step) throw new Error('Step was not found in task ' + task._taskName + '.');
                    task.on('done', task._doneHandle = handle.bind(task));
                    // bind context to current step
                    task.on('error', (task._errorHandle || root._errorHandle).bind(step));
                    if(root._debug) console.log('Start to run task: %s.', task._taskName);
                    task._run();
                }
            }
        };
