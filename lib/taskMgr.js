...
_proto.task = function(taskName) {
    ...
    task._getHandle = function(step) {
        var stepName = step.stepName;

        return step._stepHandle || task[stepName] || root[stepName] || null;
    };
    task._result = function(key, value) {
        var len = arguments.length;
        var r = root._results[task._index];

        r = r || {};

        if(!len) {return null;}
        if(value === undefined) {return r[key];}
        if(len >= 2) {
            r[key] = value;
            return root._results[task._index] = r;
        }
    };
...
_proto.step = function(stepName, stepHandle) {
    ...
    if(!stepHandle
        ...
        // add a method as `step._stepHandle` which has the same name as `step.stepName`
        // useage:
        // TaskMgr().task('foo').step('bar').bar(handle[, *args])
        ...
        var _stepHandle = function(handle) {
            ...
            if(root._currTask) {
                // this._currTask[stepName] = handle.bind(step, util.slice(arguments, 1));
                root._currTask[stepName] = handle;
            } else {
                // this[stepName] = handle.bind(step, util.slice(arguments, 1));
                root[stepName] = handle;
            }
    ...
    if(stepHandle && 'function' === typeof stepHandle) {
        // step._stepHandle = stepHandle.bind(step);
        step._stepHandle = stepHandle;
    }
 ...
 * @return this
 * @useage:
 * TaskMgr().task('foo').step('foo').error(handle)
 * var task = TaskMgr().task('foo').step('foo'); task.error = handle;
 */
...
_proto.run = function(n1, n2, n3, n4) {
    ...
    var handle = function(err) {
        if(err) {
            ...
            if(isUndefined(next) && isFunction(root._finishHandle)) {
                if(this._debug) console.log('All tasks finished, took %d ms.', Date.now() - time);
                root._finishHandle(root._results);
