...
var TaskMgr = module.exports = function(finish) {
    ...
    this._insetNames = ['debug', 'task', 'assign', 'step', 'pend', 'error', 'timeout', 'finish', 'run'];
...
_proto.step = function(stepName, stepHandle) {
    ...
    if(!stepHandle
        ...
        // can rewrite
        // && !isFunction(currTask[stepName])
        // && !isFunction(this[stepName])
    ) {
    ...
    var execEach = function(n) {
        ...
        each(taskArr, function(key) {
            ...
            task._doneHandle = function(err) {
                if(err) {
                    handle.call(root, err);
                } else {
                    if(++count >= taskArr.length) {
                        handle.apply(root, arguments);
                    }
                }
            };
