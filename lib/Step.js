...
_proto.parallel = function(arr) {
    ...
    if(isFunction(arr[0])) {
        ...
        // use the last param as callback(default is this.done)
...
_proto.jump = function(step) {
    ...
    var curStep = task._getStep(curIndex);
    ...
    if(this._debug && curStep) console.log('The step: %s has done.', curStep._stepName);

    if(targetIndex !== curIndex) {
        task._run.apply(task, [targetIndex].concat(util.slice(arguments, 1)));
    } else {
        throw new Error('The step ' + curStep._stepName + ' is executing now!');
    }
};
...
_proto.end = function(err) {
    ...
    if(this._debug) console.log('Task: %s has ended in the step: %s \
        with error: ' + (err.stack ? err.stack : err) + '.', this._taskName, this._stepName);
